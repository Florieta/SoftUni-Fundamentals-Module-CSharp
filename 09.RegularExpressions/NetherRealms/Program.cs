...
namespace NetherRealms
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] deamonNames = Console.ReadLine()
                .Split(new char[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);

            foreach (var item in deamonNames)
            {
                //Change with Regex ant MatchCollection
                int deamonHealth = item
                    .Where(c => !char.IsNumber(c) && c != '.' && c != '+' && c != '-' && c != '*' && c != '/')
                    .Sum(x => x);
